...
- **Fields** are a subset of a larger class of algebraic structures called **rings**, which are a subset of the larger class of **groups**. Each successive subset adds additional properties and is thus more complex.
...
- A more important class of finite fields, for cryptography, comprise those with $2^n$, denoted $GF(2^n)$. Which are used a wide variety of cryptographic algorithms.
...
An equivalent definition is gcd(a(x),b(x)) is the polynomial of maximum degree that divides both a(x) and b(x)
...
def inv_shiftRows(M):
    ...
    return M
```
Shiftrows is a linear function, and can be represent in matrix form $S' = P \cdot S$
#### Mix column
